...
class Abschnitt(v_max: Int = 160, länge: Int, bahnsteig: Boolean = false) {
  ...
  // let's do a quick BFS
  private def iv(todo: Queue[Abschnitt], ziel: Abschnitt): Boolean = {
    if (todo.isEmpty)
      return false
    val (a, q) = todo.dequeue
    if (a == ziel)
      return true
    val qq = if (a.weiter != null) q :+ a.weiter else q
    iv(qq ++ a.abzweige, ziel)
  }

  def ist_verbunden(ziel: Abschnitt): Boolean = iv(Queue[Abschnitt]() :+ this, ziel)
}
...
object GeradeStrecke {
  def auto_Abschnitte(v_max: Int = 160, länge: Int, a_länge: Int): GeradeStrecke = {
